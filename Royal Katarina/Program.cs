...
internal static class Program
{
    ...
    private static Obj_AI_Base toCast;
    ...
    private static ComboBox _comboStarter = new ComboBox("Combo starter", 1, new string[] { "Q", "E" });
    ...
    private static CheckBox _minionsHarass = new CheckBox("Use minions to harass enemy");
    ...
    private static CheckBox _ksWard = new CheckBox("Wardjump KS");
    ...
    private static CheckBox _drawingMinion = new CheckBox("Draw minion in harass");
    private static CheckBox _drawingHP = new CheckBox("Draw damage on HP bar");
    ...
    private static float GetComboDamage(Obj_AI_Base enemy, bool ignite = true)
    {
        float damage = 0;
        ...
        if(Player.Instance.HasItem(ItemId.Ludens_Echo))
        {
            if (Player.Instance.GetBuff("itemmagicshankcharge").Count == 100)
                damage += Player.Instance.CalculateDamageOnUnit(enemy, DamageType.Magical, 100f + (0.1f * Player.Instance.FlatMagicDamageMod));
        }

        if (ignite && igniteSlot != SpellSlot.Unknown && Player.Instance.Spellbook.CanUseSpell(igniteSlot) == SpellState.Ready)
        {
    ...
    private static void Harass()
    {
        ...
        if (_minionsHarass.CurrentValue) target = TargetSelector.GetTarget(1500, DamageType.Magical);
        else target = TargetSelector.GetTarget(Q.Range, DamageType.Magical);
    ...
    private static void KillSteal()
    {
        if (_ksSmart.CurrentValue)
        {
            ...
            foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(x => !x.IsInvulnerable && !x.IsZombie && !x.IsDead && x.IsEnemy && x.Distance(Player.Instance) < E.Range).OrderBy(x => x.Health))
            {
            ...
            if (!E.IsReady() || !_ksWard.CurrentValue) return;

            foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(x => !x.IsInvulnerable && !x.IsZombie && !x.IsDead && x.IsEnemy && x.Distance(Player.Instance) < 600 + W.Range).OrderBy(x => x.Health))
            {
                float damageCanBeDone =
                    (Q.IsReady() ? Player.Instance.GetSpellDamage(enemy, SpellSlot.Q) : 0) +
                    (W.IsReady() ? Player.Instance.GetSpellDamage(enemy, SpellSlot.W) : 0);
            ...
            foreach (var enemy in ObjectManager.Get<AIHeroClient>().Where(x => !x.IsInvulnerable && !x.IsZombie && !x.IsDead && x.IsEnemy && x.Distance(Player.Instance) < 600 + Q.Range).OrderBy(x => x.Health))
            {
                float damageCanBeDone =
                    (Q.IsReady() ? Player.Instance.GetSpellDamage(enemy, SpellSlot.Q) : 0);
                if (damageCanBeDone > enemy.Health)
    ...
    private static void MenuLoad()
    {
        ...
        MenuMisc.AddLabel("Killsteal settings");
        MenuMisc.Add("_ksSmartR", _ksSmart);
        MenuMisc.Add("_ksRR", _ksR);
        MenuMisc.Add("_ksWardR", _ksWard);
        ...
        Drawings.Add("_drawingMinionR", _drawingMinion);
        Drawings.Add("_drawingHPR", _drawingHP);
    }
    ...
    {
        var target = TargetSelector.GetTarget(Q.Range, DamageType.Magical);
        if (target == null || target.IsDead || target.IsInvulnerable || !target.IsValid || Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo) || HasRBuff())
        {
            return;
        }
    ...
    {
        if (!_drawingHP.CurrentValue || Player.Instance.IsDead) return;
        foreach (var unit in EntityManager.Heroes.Enemies.Where(u => u.IsValidTarget() && u.IsHPBarRendered))
        {
            ...
            System.Drawing.Color colorH;
            if (damage < unit.Health) colorH = System.Drawing.Color.FromArgb(Color.Yellow.A - 120, Color.Yellow.R, Color.Yellow.G, Color.Yellow.B);
            else colorH = System.Drawing.Color.FromArgb(Color.LightGreen.A - 120, Color.LightGreen.R, Color.LightGreen.G, Color.LightGreen.B);
    ...
    private static void Drawings(EventArgs args)
    {
        if (Player.Instance.IsDead)
            return;
        if (_drawingMinion.CurrentValue && Orbwalker.ActiveModesFlags == Orbwalker.ActiveModes.Harass && _minionsHarass.CurrentValue && toCast != null && !toCast.IsDead && toCast.IsVisible) //&& willHit.Count > 0 && Orbwalker.ActiveModesFlags == Orbwalker.ActiveModes.Harass)
        {
            Circle.Draw(Color.Cyan, 60, toCast.Position);
        }
    ...
    private static void OnUpdate(EventArgs args)
    {
        ...
        foreach (var buff in Player.Instance.Buffs)
            Chat.Print(buff.Name + " " + buff.Count);
        */
        if (HasRBuff() && TargetSelector.GetTarget(R.Range, DamageType.Magical) != null)
        {
            Orbwalker.DisableMovement = true;
            Orbwalker.DisableAttacking = true;
        }
    ...
    private static void WardJump(Vector3 pos, bool m2M = true, bool maxRange = true, bool reqinMaxRange = false, bool minions = true, bool champions = true)
    {
        if (!E.IsReady())
        {
            return;
    ...
    static void CastQHarass(AIHeroClient target)
    {
        if (Q.IsInRange(target)) Q.Cast(target);
        else if (_minionsHarass.CurrentValue)
        {
            var minions = ObjectManager.Get<Obj_AI_Base>().Where(x =>
                x.IsEnemy &&
                !x.IsDead &&
                !x.IsWard() &&
                !x.IsStructure() &&
                x.IsTargetable &&
                x.Distance(target) < 1500 &&
                Player.Instance.Distance(x.Position) < Player.Instance.Distance(target.Position)).
                OrderBy(x => x.Distance(Game.CursorPos)).ToArray();
            
            if (minions.Length == 0) return;

            try
            {
                List<Obj_AI_Base> toHit = new List<Obj_AI_Base>(6);
                float bestThrowDistance = 1500;
                ...
                for (int i = 1; i < minions.Length; i++)
                {
                    ...
                    for (int j = 1; j < 5; j++)
                    {
                        Obj_AI_Base nearest = ObjectManager.Get<Obj_AI_Base>().Where(x =>
                            x.IsEnemy &&
                            !x.IsDead &&
                            !x.IsWard() &&
                            !x.IsStructure() &&
                            x.IsTargetable &&
                            !toHit.Contains(x) &&
                            x.Distance(toHit[j - 1]) <= 625).
                            OrderBy(x => x.Distance(toHit[j - 1])).FirstOrDefault();
                    ...
                    if (toHit[0].Distance(Player.Instance.Position) < bestThrowDistance && ableToHit)
                    {
                        bestThrowDistance = toHit[0].Distance(Player.Instance.Position);
                        bestTarget = toHit[0];
                    }
                }
                ...
                toCast = bestTarget;
                if (Q.IsInRange(toCast)) Q.Cast(toCast);
            }
            catch (Exception ex)
            {
